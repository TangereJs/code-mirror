...
(function(mod) {
  ...
  CodeMirror.defineMode("liquid-json", function (config, parserConfig) {
    var jsonModeSpec = CodeMirror.resolveMode("application/json");
    var jsonMode = CodeMirror.getMode(config, jsonModeSpec);
    ...
    function parseInJsonMode(stream, state) {

      if(!state.localState) {
        state.localState = jsonMode.startState();
      }
      if (!state.localMode) {
        state.localMode = jsonMode;
      }
    ...
    function lmToken(stream, state) {
      ...
      // return parseInJsonMode(stream, state);
      ...
      if (currentParsingModeFn !== null && !isStartOfLiquid) {
        ...
        if (isStartOfLiquid) {
          ...
          currentParsingModeFn = parseInJsonMode;
          style = currentParsingModeFn(stream, state);
  ...
  }, "application/json", "carbonliquid");

  CodeMirror.defineMIME("text/liquid-json", "liquid-json");
});
